...
/*!positions/page/my-positions-list-offline/main.js*/
;/**
 * B端主页业务入口文件
 * 此文件应当只包含：
 *     require( 'xxx' );
...
define('positions/page/my-positions-list-offline/main', ['require', 'exports', 'module', 'common/widgets/header/main', 'common/widgets/navigation/main', 'dep/jquery-placeholder/jquery.placeholder', 'common/components/pagination/jquery.pagination', 'common/static/js/lagou.mini', 'common/components/filterSelect/main', 'common/components/filterGridView/main', 'common/components/memberCheckBoxList/main', 'positions/modules/plus-positon-tips-widget/main', 'positions/modules/add-recruiter-pop/main', 'positions/modules/group-share-position-pop/main'], function(require, exports, module) {
 ...
 $('.add-recruiter').on('click',function(e){
            ...
            $.ajax({
                ...
                cache:false
                    ...
                    if(data.state == 1){
                        ...
                        //             lg.AllMember[i].isChecked=true;
                        //             myMembers.push(lg.AllMember[i]);
                        //         }
                        //     }
    ...
    $('.recruiter-reflash').on('click',function(e){
        var that = this;
        if($(that).text()!=freshedText){
            $.ajax({
                url: 'refreshPosition.json',
                dataType: 'json',
                data:{positionId:$(this).closest('li').attr('data-item')},
                type:'POST',
                cache:false
            }).done(function(result) {
                if(result.state==1){
                    $($($(that).closest('.position-item-bottom').children('div')[0]).find('span')[0]).text('刚刚');
                    $(that).text(freshedText).css('color','#c1c1c1');
                }else{
                    alert('刷新职位失败！');
                }
    ...
    //下线职位再发布
    $(".republish-offline").on('click', function (e) {
        var confirmText = '';
        ...
        if ( goodsInfo.onlinePositionCount >= goodsInfo.onlinePositionMaxCount ) {
            if ( goodsInfo.lagouJiaCompany && goodsInfo.miniLagouJia && goodsInfo.pause ) {
                confirmText = '由于贵公司拉勾网在线职位数已达上限，<em style="color:#0099ff;">暂不可将职位再发布到拉勾网。</em>请下线部分职位以发布更多新职位。';
            } else if ( goodsInfo.lagouJiaCompany ) {
                confirmText = '由于贵公司拉勾网在线职位数已达上限，<em style="color:#0099ff;">暂不可将职位再发布到拉勾网。</em>请下线部分职位或联系拉勾客户经理获取更多在线职位数。';
            } else {
                confirmText = '由于贵公司拉勾网在线职位数已达上限，<em style="color:#0099ff;">暂不可将职位再发布到拉勾网。</em>请下线部分职位或升级拉勾+ 获取更多在线职位数。';
            }
        }
        ...
        var $me = $(this);
        var type = parseInt($me.attr('data-account-type'), 10);

        if ( type === 1 || type === 2 ) { // 大帐号、小帐号跳转主站hr
            return true;
        }

        e.preventDefault(); // 其他帐号执行简招现有流程

        if ( $me.attr('disabled') !== 'disabled' ) {
            var confirm = new lg.Widgets.Controls.Confirm({
                content: '系统将直接发布该职位，如需编辑职位描述，可在发布后进行编辑。',
                submitText: '好的',
                cancelText: '取消',
                SubmitBtn: function(e){
                    $.ajax({
                        url: 'republishOfflinePosition.json',
                        dataType: 'json',
                        data: {
                            positionId: $me.closest('li.position-list-item').attr('data-item')
                        },
                        type: 'POST',
                        cache: false
                    }).done(function (result) {
                        $me.css('color', '#c1c1c1').attr('disabled', 'disabled');

                        var state = parseInt(result.state, 10);
                        if ( state === 1 ) {
                            $me.text("职位发布中");
                            window.location.reload();
                        } else if ( state === 201 || state === 402 ) {
                            $me.text("该职位无法再发布");
                        } else if ( state === 202 ) {
                            $me.text("职位数已达上限");
                        } else if( state === 40301){
                              var confirm = new lg.Widgets.Controls.Confirm({
                                content:'<h2 style="color:#333;text-align:center;color:#333;">未完成认证无法发布职位</h2><p style="text-align:center;font-size:14px;color:#777;">打开拉勾网APP，在“个人页面”完成招聘者身份的认证</p><p style="text-align:center;font-size:14px;color:#777;">通过拉勾网审核后即可发布新职位。</p>',
                                    submitText:"知道了",
                                    noCancelBtn:true,
                                    SubmitBtn:function(e){
                                        e.control.setRemove();
                              ...
                              confirm.getElement().css({width:"400px"});
                        }else {
                            $me.text("再发布失败");
                        }
